  ...
  <CrudForm :width="'calc(100% - ' + (280+12) + 'px)'">
    ...
    <template v-slot:header>
      <CrudHeader
          ...
          @add-event="showModal = !showModal" @update-event="editMetadata(currentRow)" @delete-event="delConfirm"
      />
    </template>
    ...
    <el-dialog v-model="active" :before-close="dialogVisible" :width="600">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <n-button type="primary" color="#0099CB" style="margin-right: 10px" @click="debugApi">开始调试</n-button>
    ...
    <el-dialog v-model="showModal" :width="700">
      ...
      <n-data-table
          bordered
          :single-line="false"
          :columns="[ {type: 'selection',multiple: false,},{title: 'API类型',key: 'API类型'},{title: '描述',key: '描述'}]"
          :data="[ {key: '自定义SQL', API类型: '自定义SQL',描述: '支持编写自定义查询SQL,直接查询数据源数据，生成数据服务API',},{key: '注册API', API类型: '注册API',描述: '支持将已有Web服务注册到平台进行统一管理'}]"
          @update:checked-row-keys="handleCheck"
          :default-checked-row-keys="['自定义SQL']"
          style="margin-top: 10px"
      />
      <template #footer>
        <n-button color="#0099CB" type="primary" size="small" @click="createApi">确定</n-button>
      </template>
...
const updateApiTreeUrl = import.meta.env.MODE === 'development'
    ? import.meta.env.VITE_APP_DEV_API_URL+'/HDataApi/interface/updateInterfaceFloderRename'
    : window.webConfig.VITE_APP_PROD_API_URL+'/HDataApi/interface/updateInterfaceFloderRename'
...
function updateMenu(ruleFormRef) {
  ...
  axios.post(updateApiTreeUrl, params).then((res) => {
    message.info(res.data.info)
...
function updateApiTimeConsuming (id, executeTime) {
  let url = import.meta.env.MODE === 'development'
      ? import.meta.env.VITE_APP_DEV_API_URL+'/HDataApi/interface/updateApiTimeConsuming'
      : window.webConfig.VITE_APP_PROD_API_URL+'/HDataApi/interface/updateApiTimeConsuming'
  let param = {
    apiId: id,
