<template>
  ...
  <div class="m-sub-process-box">
    ...
    <div class="right-bar"><div @click="openTab('first')">任务属性</div></div>
        <div style="height: 100%;margin: 30px 250px">
          <NForm :disabled="props.readOnly" size="small" label-placement="left" require-mark-placement="left" label-align="right" label-width="120" :rules="rules" :model="taskData" ref="formRef">
            <NGrid x-gap="10">
              <NFormItemGi
...
const json = [
  Fields.useName(0, true),
  Fields.useTaskType(taskData.value, true),
  Fields.useChildNode({
    model: taskData.value,
...
const elements = computed(() => {
  return getElementByJson(json, taskData.value).elements;
})

const rules = computed(() => {
  return getElementByJson(json, taskData.value).rules;
})
